...
@WiContrib({})
@Injectable()
export class SQLConnectorContribution extends WiServiceHandlerContribution {
    constructor() {
        super();
    }
    ...
    validate = (name: string, context: IConnectorContribution): Observable<IValidationResult> | IValidationResult => {
		...
		if(name === "Connect") {
			...
			for (let configuration of context.settings) {
				if( configuration.name === "vendor") {
					vendor = configuration.value	
				} else if( configuration.name === "url") {
					url = configuration.value
				} else if( configuration.name === "user") {
					user = configuration.value
				} else if( configuration.name === "password") {
					password = configuration.value
				} else if( configuration.name === "resources") {
					resources = configuration.value
				}
			}
