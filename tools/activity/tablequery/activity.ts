    ...
    value = (fieldName: string, context: IActivityContribution): Observable<any> | any => {
        ...
        if (fieldName === "Table") {
            ...
            return Observable.create(observer => {
        		...
        		return buildQueryKey(this.http, this.selectedConnector, (key : string, schema : string) => {
					console.log("Key = " + key)
            		var attrJsonKey = {};
            		if (schema) {
						...
						if(key==data[i].Name) {
                				attrJsonKey[data[i].Name] = populateAttribute(data[i].Type);
						}
                		}
            		}
            		return attrJsonKey;
...
function buildQueryKey(http, selectedConnector, builder) : Observable<any> {
	return Observable.create(observer => {
		WiContributionUtils.getConnections(http, "GraphBuilder_Tools").subscribe((data: IConnectorContribution[]) => {
			...
			observer.next(JSON.stringify(builder(key, schema)));
			observer.complete();
		});
	});			
}
