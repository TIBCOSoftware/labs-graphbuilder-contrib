...
@WiContrib({})
@Injectable()
export class DgraphContributionHandler extends WiServiceHandlerContribution {
	url: string;
	user: string; 
	passsword: string; 
    ...
    validate = (fieldName: string, context: IActivityContribution): Observable<IValidationResult> | IValidationResult => {
        ...
        if (fieldName === "dgraphConnection") {
            let connection: IFieldDefinition = context.getField("dgraphConnection")
        	if (connection.value === null) {
            	return ValidationResult.newValidationResult().setError("DGraph-MSG-1000", "Dgraph must be configured");
        	}
        }
		return null; 
    }
