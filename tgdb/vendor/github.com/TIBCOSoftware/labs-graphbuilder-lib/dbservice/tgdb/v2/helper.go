...
func BuildResult(tgdb *TGDBService, tgResult map[string]interface{}) map[string]interface{} {
	...
	if nil != tgResult {
		...
		if nil != tgResult["nodes"] {
			...
			//fmt.Print("(1)")
			for _, entity := range tgResult["nodes"].(map[int64]types.TGEntity) {
		...
		if nil != tgResult["edges"] {
			...
			//fmt.Print("(8)")
			for _, entity := range tgResult["edges"].(map[int64]types.TGEntity) {
				...
				if nil != edge {
					if nil != edge.GetEntityType() &&
						...
						fromNodeId := ExtractNodeKeyAttrValue(tgdb, fromNode)
						//fmt.Print("(9.2)")
						toNode := edge.GetVertices()[1]
						toNodeId := ExtractNodeKeyAttrValue(tgdb, toNode)
						//fmt.Print("(10)")
...
func ExtractEdgeKey(tgdb *TGDBService, edge types.TGEdge) []interface{} {
