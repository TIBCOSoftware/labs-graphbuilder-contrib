...
func (this *TGDBServiceFactory) Initialize() {
	logging.DefaultTGLogManager().GetLogger().SetLogLevel(types.ErrorLog)
}
...
func (this *TGDBServiceFactory) CreateUpsertService(serviceId string, properties map[string]interface{}) (dbservice.UpsertService, error) {
	...
	if nil == tgdbService {
		...
		tgdbService._nodeAttrMap = make(map[string]map[string]types.TGAttributeDescriptor)
		tgdbService._edgeAttrMap = make(map[string]map[string]types.TGAttributeDescriptor)
