...
@WiContrib({})
@Injectable()
export class SSEEndpointContributionHandler extends WiServiceHandlerContribution {
	filename: string;
	content: string; 
        ...
        if (fieldName === "sseConnection") {
            let connection: IFieldDefinition = context.getField("sseConnection")
        		if (connection.value === null) {
        			return ValidationResult.newValidationResult().setError("SSE-ENDPOINT-MSG-1000", "Connector must be configured");
        		}
        }
		return null; 
    }
